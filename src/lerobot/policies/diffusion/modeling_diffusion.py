...
class DiffusionControlnetPolicy(PreTrainedPolicy):
    """
    Diffusion Policy as per "Diffusion Policy: Visuomotor Policy Learning via Action Diffusion"
    (paper: https://huggingface.co/papers/2303.04137, code: https://github.com/real-stanford/diffusion_policy).
    """
    ...
    def __init__(
        ...
        self.diffusion = DiffusionControlnetModel(config)
...
class DiffusionControlnetModel(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        self.unet = DiffusionConditionalUnet1d(config, global_cond_dim=global_cond_dim * config.n_obs_steps)
        self.control_net = ControlNet(config, global_cond_dim=global_cond_dim * config.n_obs_steps)
...
class ControlNet(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig, global_cond_dim: int):
        ...
        # ControlNet encoder.
        self.copy_down_modules = nn.ModuleList([])
        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= (len(in_out) - 1)
            self.copy_down_modules.append(
                nn.ModuleList(
                    [
                        DiffusionConditionalResidualBlock1d(dim_in, dim_out, **common_res_block_kwargs),
                        DiffusionConditionalResidualBlock1d(dim_out, dim_out, **common_res_block_kwargs),
        ...
        # Processing in the middle of the auto-encoder.
        self.copy_mid_modules = nn.ModuleList(
            [
                DiffusionConditionalResidualBlock1d(
                    config.down_dims[-1], config.down_dims[-1], **common_res_block_kwargs
                ),
        ...
        self.mid_controlnet_block = self.zero_module(
            nn.Conv1d(config.down_dims[-1], config.down_dims[-1], 1)
        )
    ...
    def forward(self, x: Tensor, timestep: Tensor | int, global_cond=None, control_input=None) -> Tensor:
        ...
        if control_input is not None:
            ...
            encoder_skip_features_hat: list[Tensor] = []
            for resnet, resnet2, downsample in self.copy_down_modules:
                x_hat = resnet(x_hat, global_feature)
                x_hat = resnet2(x_hat, global_feature)
                encoder_skip_features_hat.append(x_hat)
                x_hat = downsample(x_hat)
            for mid_module in self.copy_mid_modules:
            ...
            """
            for resnet, resnet2, upsample in self.up_modules:
                x = x + encoder_skip_features_hat.pop()
                x = torch.cat((x, encoder_skip_features.pop()), dim=1)
                x = resnet(x, global_feature)
